		...
		int optname, char __user *optval, unsigned int optlen)
{
	...
	case MPTCP_PATH_MANAGER: {
		...
		return err;
	//CX ADD FOR RELES
	case MPTCP_SPLIT_RATIO:{
		/* Cannot be used if MPTCP is not used or MPTCP connection has not
		* been established or not reles schduler.  
		* not sure!! if err , check whether sk_state can be used like this.
		 */
		char ratio[64];
		if (mptcp_init_failed || 
		    !sysctl_mptcp_enabled ||
		    sk->sk_state != TCP_ESTABLISHED || 
			((strcmp(tcp_sk(sk)->mptcp_sched_name,"reles")!=0) && (strcmp(tcp_sk(sk)->mptcp_sched_name,"random")!=0)))
			return -EPERM;
		
		val = strncpy_from_user(ratio, optval,
					min_t(long, 64,
					      optlen));
		if (val < 0)
			return -EFAULT;
		ratio[val] = 0;
		//pr_info("[tcp.c]optival: %s \n", optval);
		//pr_info("[tcp.c]ratio: %s \n", ratio);
		lock_sock(sk);
		//err = mptcp_set_split_ratio(sk, ratio);
		//float ratio[MPTCP_PNUM_MAX];
		//char2float(optval,ratio,sizeof(optval),sizeof(ratio);
		if(strcmp(tcp_sk(sk)->mptcp_sched_name,"reles")==0)
			err = mptcp_set_split_ratio(sk,ratio,optlen);
		if((strcmp(tcp_sk(sk)->mptcp_sched_name,"random")==0))
		{
			err = random_mptcp_set_weight(sk,ratio,optlen);
...
/*
int char2float(char _user *optval,float *array,u8 len_opt, u8 len_array)
{
	char *tmp="";
	int i =0;
	int tmp_inx = 0;
	...
	for(;i<len_opt;i++)
	{
		...
		else
		{
			*tmp = '\0';
			tmp_inx = 0;
			if(float_len>=len_array)
				break;
			*(array+float_len++)=atof(tmp);
		}
	}
}
*/
